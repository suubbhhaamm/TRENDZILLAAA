...
def get_favoriteProductsByReviews(user):
    ...
    try:
        connection = psycopg2.connect(user="yashika06",
                                      password=os.environ.get('DB_PASS'),
                                      host="db-instance.cltrcjaspfh8.ap-south-1.rds.amazonaws.com",
                                      port="5432",
                                      database="trendzilla_db")
        

        cursor = connection.cursor()
        postgreSQL_select_Query = r"select * from (select base_product.category, count(base_product.category) as FavoriteProducts from base_review inner join base_product on base_product._id = base_review.product_id where (lower(base_review.comment) like '%nice%' or lower(base_review.comment) like '%good%' or lower(base_review.comment) like '%excellent%' or lower(base_review.comment) like '%gorgeous%' or lower(base_review.comment) like '%comfortable%' or lower(base_review.comment) like '%beautiful%' or lower(base_review.comment) like '%awesome%') group by base_product.category) A order by A.FavoriteProducts desc"
